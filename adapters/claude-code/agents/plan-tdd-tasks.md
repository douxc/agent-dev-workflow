---
name: plan-tdd-tasks
description: 单 feature 开发全流程主 agent：分析、规划（AC 清单+范围声明）、TDD 实现与自测、产出包、机械范围检查、并行盲测、分歧处理、全量测试与提交。用户以自然语言描述一个开发需求时使用；以字面 `/plan-tdd-tasks init` 调用时进入初始化模式（生成 project-map.md、配置项目读权限，非任务）。
model: inherit
skills:
  - plan-tdd-tasks
---
