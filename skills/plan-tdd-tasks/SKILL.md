---
name: plan-tdd-tasks
description: 单 feature 开发全流程主 agent：分析、规划（AC 清单与范围声明）、准入闸门、TDD 实现与自测、机械范围检查、全量测试、产出包、并行盲测复核、分歧处理与提交。用户描述一个开发需求时使用；一次只处理一个 feature。
---

# 单 Feature 全流程：分析 → 规划 → TDD → 盲测 → 全量

本 skill 是单个 feature 开发任务的唯一执行者。所有工作在同一个会话中完成：分析、规划、准入闸门、TDD 实现与自测、机械范围检查、全量测试、产出包、并行盲测、分歧处理与提交。本 skill 无状态机、无版本号、无 gates；稳定性来自 AC 清单的质量、范围声明的机械检查与两个全新上下文的盲测者。本 skill 在业务仓库维护的唯一持久化项目元数据是 `${PROJECT_ROOT}/project-map.md`（§11）——它是项目的索引（地图），不承载任何规划状态。
...
→ ④ TDD 实现与自测 → ⑤ check-scope.sh 机械范围检查 → ⑥ 全量测试（恰好 1 遍）+ 产出包
→ ⑦ 盲测阶段（盲测 ×2，并行、只读、全新上下文）
...
- `${SKILL_ROOT}` 是包含当前已加载 `SKILL.md` 的目录。本 skill 的脚本固定为 `${SKILL_ROOT}/scripts/` 下八个：`check-scope.sh`（§6/§9）、`build-package.sh`（§6）、`run-full-tests.sh`（§6）、`decide-verdicts.sh`（§8）、`stage-scope.sh`（§9）与 `check-env.sh`、`validate-ac.sh`、`parse-verdict.sh`（§4.4/§7），**以绝对路径调用**；禁止在业务仓库复制、改写或新建这些脚本，禁止探索业务仓库中的同名文件。
...
恰好一行非空内容：§3 确定的全量测试命令；用户同意跳过测试执行时写入字面 `SKIP`（机器可识别，§6 检测到即跳过执行）。
...
- 迭代期间只运行**定向测试**（正在写的那个或邻近的测试），不跑全量；全量在 §6 提交盲测前运行恰好 1 遍。
...
2. **全量测试（提交盲测前的项目级验证）**：TDD 完成、提交盲测前运行，每次尝试最后执行、恰好 1 遍（绝对路径，日志 `full-tests.log` 供 §9 提交门禁读取）：
...
`decide-verdicts: DOUBLE-PASS` → §9 提交；`decide-verdicts: DOUBLE-FAIL` → 双 fail（下）；`decide-verdicts: SPLIT` → 相左（下）；`decide-verdicts: MALFORMED (<文件>)` → 只重派那一个盲测者（§7 末条，全新上下文）。
...
- **双 fail**：只读取两份 verdict 的证据，**禁止反驳**（不辩解、不质疑检查项合理性）。逐条修复后回到 §6（范围复检 → 全量测试 → 构建代码包）并重派盲测（全新上下文），轮次 +1。若修复过程中发现需要改变范围或新增 AC，先走 §6 的回退重规划。
...
- §6 全量测试失败后，只有代码包事实变化而触发的重派计入轮次上限；环境或测试命令修复不消耗轮次。
