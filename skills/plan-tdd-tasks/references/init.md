...
2. **权限**：展示规则文本 `` `Read(<PROJECT_ROOT>/**)` ``；经用户同意后调用 `update-config` skill 写入 `.claude/settings.local.json` 的 `permissions.allow`，拒绝则跳过。若 skill 不可用，精确报告 `update-config unavailable; permission step skipped`，跳过权限写入并继续其余步骤。
...
4. **收尾**：展示待提交的 `project-map.md` 与可能的 `.gitignore` 变更，只执行一次 commit。地图新建使用 `chore: init project-map`；地图更新使用 `chore: update project-map`。汇报地图生成、更新或跳过状态、权限结果和 commit sha；settings.local.json 不进入提交。
