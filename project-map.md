...
- 两个成对安装的 skill：`skills/plan-tdd-tasks/`（单 feature 开发全流程主 skill：分析 → 规划（AC 清单 + 范围声明 + test-command.txt）→ TDD → 产出包 + 机械范围检查 → 并行盲测 ×2 → 分歧处理 → 全量测试 → 提交）与 `skills/blind-review-tasks/`（纯只读静态盲审复核者，只由主 skill 并行派发）；盲审传输按宿主选择 Claude Code `Agent(blind-review-tasks)` 或 Hermes `delegate_task`，传输不可用时 fail closed。
